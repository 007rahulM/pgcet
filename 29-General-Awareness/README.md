# General Awareness — Overview

This folder now follows the same structured style as the other strong folders: **recognition clues, topic mapping, quick teaching notes, practice questions, and separate answer files**.

General Awareness in PGCET is not about memorizing everything. It is about covering the syllabus buckets clearly and revising them in a repeatable way.

## Patterns in This Folder

| File | Pattern | Recognition Clue |
|------|---------|-----------------|
| [Pattern1-General-Awareness.md](./Pattern1-General-Awareness.md) | Full syllabus overview | "Mixed GK set" |
| [Pattern2-Business-and-Finance.md](./Pattern2-Business-and-Finance.md) | Banking, economy, finance, markets | "repo rate", "GDP", "SEBI", "UPI" |
| [Pattern3-Governance-and-Polity.md](./Pattern3-Governance-and-Polity.md) | Constitution, institutions, governance | "President", "ECI", "CAG", "Parliament" |
| [Pattern4-Science-Tech-and-Health.md](./Pattern4-Science-Tech-and-Health.md) | Tech, science, healthcare, innovation | "WHO", "ISRO", "AI", "renewables" |
| [Pattern5-Culture-Entrepreneurship-and-Communication.md](./Pattern5-Culture-Entrepreneurship-and-Communication.md) | Culture, startup terms, business communication | "classical dance", "MSME", "minutes of meeting" |
| [Pattern6-2025-Emerging-Terms.md](./Pattern6-2025-Emerging-Terms.md) | 2025-style new terms (finance, transport, health models, ADAS) | "strike price", "cabotage", "LEARN", "LiDAR" |

## Golden Rules

- Build category-wise memory, not random memory.
- Revise full forms, institutions, and one-line definitions daily.
- For current-affairs-type questions, connect the term to its domain first.
- Practice through short repeated recall, not long passive reading.

## Study Order

1. Pattern 1 for the map.
2. Pattern 2 and 3 for the most exam-like one-liners.
3. Pattern 4 and 5 for mixed static + modern awareness.
4. Pattern 6 for 2025-style emerging one-liner terms.
